...
/**
 * Auto-infers baseline column roles based on column name heuristics and regex patterns. Migrated
 * from incognito to Effigies to preserve fail-closed engine execution.
 */
class PolicyInferrer {
  ...
  private static final Pattern PHONE_PATTERN =
      Pattern.compile("(?i).*(phone|mobile|fax)(_?number)?$");
  private static final Pattern NAME_PATTERN =
      Pattern.compile("(?i).*(first_?name|last_?name|surname|full_?name).*");
  private static final Pattern DOB_PATTERN =
      Pattern.compile("(?i).*(dob|birth_?date|date_of_birth).*");
  private static final Pattern NATIONAL_ID_PATTERN =
      Pattern.compile("(?i).*(ssn|social_?security|tax_?id|nhs_?num|nino|national_?insurance).*");
  private static final Pattern PASSPORT_PATTERN =
      Pattern.compile("(?i).*passport(_?number|_?no)?$");
  private static final Pattern DRIVING_LICENCE_PATTERN =
      Pattern.compile("(?i).*driv(ing|er)_?licen[cs]e(_?number|_?no)?$");
  private static final Pattern CREDIT_CARD_PATTERN =
      Pattern.compile("(?i).*(credit_?card|card_?number|cc_?number)$");
  ...
  private static final Pattern POSTCODE_PATTERN =
      Pattern.compile("(?i).*(post_?code|postal_?code|zip_?code|zip)$");
  ...
  private static final Map<String, DirectIdStrategy> STRATEGY_BY_HEURISTIC =
      Map.of(
  ...
  Optional<InferredRole> inferRole(String columnName) {
    if (EMAIL_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "EMAIL_PATTERN"));
    }
    if (PHONE_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "PHONE_PATTERN"));
    }
    if (NAME_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "NAME_PATTERN"));
    }
    if (NATIONAL_ID_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "NATIONAL_ID_PATTERN"));
    }
    if (PASSPORT_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "PASSPORT_PATTERN"));
    }
    if (DRIVING_LICENCE_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.DIRECT_ID, "DRIVING_LICENCE_PATTERN"));
    }
    ...
    if (CREDIT_CARD_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.SENSITIVE, "CREDIT_CARD_PATTERN"));
    }
    if (POSTCODE_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.QUASI_ID, "POSTCODE_PATTERN"));
    }
    if (DOB_PATTERN.matcher(columnName).matches()) {
      return Optional.of(new InferredRole(ColumnRole.QUASI_ID, "DOB_PATTERN"));
    }
   ...
   * scaffold's {@code directIdStrategy} stub - empty if the heuristic doesn't determine one typed
   * generator (e.g. {@code NATIONAL_ID_PATTERN}, which spans several).
   */
