...
class ScaffoldCommand {
   ...
   */
  static int run(
      DataSource dataSource, File file, boolean force, PrintStream out, PrintStream err) {
  ...
  static void writeScaffold(File file, List<SchemaInspector.TableMetadata> tables)
      throws IOException {
    ...
    try (BufferedWriter writer = Files.newBufferedWriter(file.toPath(), StandardCharsets.UTF_8)) {
      ...
      writer.write(
          "# The `type:` shown against each column below is JDBC's own name, not necessarily "
              + "the\n");
      writer.write(
          "# database's - e.g. PostgreSQL's BOOLEAN reports here as JDBC's BIT, and TEXT as "
              + "VARCHAR.\n");
      writer.write(
          "# Still a reliable input for choosing a strategy, just not identical to what the DDL "
              + "says.\n");
      writer.write("tables:\n");
  ...
  /**
   * Writes the {@code role:} TODO stub for one column, plus - where the choice determining output
   * quality is itself known or inferable - a second stub for it ({@code directIdStrategy}, {@code
   * distinguishing}, {@code references}, {@code surrogateStrategy}): never assigned, only
   * suggested, the same "suggest, never assign" pattern as {@code role:} itself. Structural facts
   * from {@link SchemaInspector} (a column IS the primary key, or IS a foreign key) take priority
   * over {@link PolicyInferrer}'s name-based heuristics - a known constraint outranks a guess.
   */
  private static void writeRoleStub(
      BufferedWriter writer,
      SchemaInspector.TableMetadata table,
      String col,
      Map<String, SchemaInspector.TableMetadata> tablesByName,
      PolicyInferrer inferrer)
      throws IOException {

    if (table.foreignKeys().containsKey(col)) {
      String parentTable = table.foreignKeys().get(col);
      writer.write(
          "        role:              # TODO classify (Suggestion: FOREIGN_KEY -> "
              + parentTable
      ...
      if (parent != null && parent.primaryKeyColumns().size() == 1) {
        ...
        writer.write(
            "        references:        # TODO if FOREIGN_KEY - target table is "
                + parentTable
    ...
    if (table.primaryKeyColumns().contains(col)) {
      writer.write(
          "        role:              # TODO classify (Suggestion: PRIMARY_KEY,"
              + " structurally discovered - not a guess)\n");
      writer.write(
    ...
    if (inferred.isEmpty()) {
      writer.write(
          "        role:              # TODO classify - see docs/spec/incognito.md"
    ...
    if (role.role() == ColumnRole.DIRECT_ID) {
      ...
      if (strategy.isPresent()) {
        writer.write(
            "        directIdStrategy:  # TODO if DIRECT_ID (Suggestion: "
                + strategy.get()
                + ")\n");
      } else {
        writer.write(
            "        directIdStrategy:  # TODO if DIRECT_ID - see DirectIdStrategy's"
                + " Javadoc for the right typed generator\n");
      }
    } else if (role.role() == ColumnRole.SENSITIVE) {
      writer.write(
          "        distinguishing:    # TODO if SENSITIVE (true|false - does this alone identify "
              + "someone?)\n");
      ...
      if (strategy.isPresent()) {
        writer.write(
            "        directIdStrategy:  # TODO if QUASI_ID (Suggestion: " + strategy.get() + ")\n");
      }
    }
  }
}
