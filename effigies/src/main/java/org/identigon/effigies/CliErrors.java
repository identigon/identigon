...
final class CliErrors {
  ...
  /**
   * Renders {@code t} followed by every {@code getCause()} beneath it, one per line, each prefixed
   * {@code "Caused by: "} - short {@code toString()} entries (class + message), not full stack
   * traces, matching this CLI's terse error style.
   *
   ...
   * already prints {@code t}'s own message on its own terms (e.g. a fail-closed {@code
   * ConfigException}, whose message alone is already the clean diagnostic) and only wants to reveal
   * a hidden cause, if {@code t} has one.
   *
   * @param t the top-level exception
