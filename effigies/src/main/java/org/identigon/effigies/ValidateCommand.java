...
/**
 * Checks a policy against a source schema with no target connection and no data movement -
 * {@code SchemaDiscoveryStage}'s fail-closed diagnostics (the tool's best, per SPEC §7.2) reachable
 * without committing to a full {@code run}. Useful while authoring, and as a CI pre-flight check for
 * a policy going stale after a schema migration.
 */
class ValidateCommand {
     ...
     * out from {@link #execute} so tests can exercise it directly without needing to fake
