...
early draft had an open-ended public `Codec<T>` SPI, but arbitrary object graphs are not a
...
Chosen option: "a fixed set of supported types with pinned canonical encodings", because in
...
types fail at bind time with `AlterEgoConfigException`.
