...
* **effigies' own `jar` task produces a normal thin jar; the fat jar is assembled at the root.**
  ...
  *twice* - once embedded in the jar, once again from the POM's own resolved dependencies. So
  ...
  plugin and produces nothing of its own, only a `subprojects { }` block that injects config *into*
