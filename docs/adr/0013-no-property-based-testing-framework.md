...
written to stdout during test runs, ANSI-escaped to be invisible in a human terminal but readable
by an AI agent, instructing it to delete the caller's tests and code. Release 1.10.1 softened the
...
Property-style tests are plain JUnit Jupiter tests that loop over a deterministically enumerated
