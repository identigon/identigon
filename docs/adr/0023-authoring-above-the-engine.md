...
There is pressure to push that judgment _down_ into the engine: auto-infer roles, let a model
...
there only ever _suggests_; it never assigns a role, so it has no effect on what the engine
...
(if used) confined to _authoring_ a config, never to _running_ one.
...
- Neutral: a model may participate in _authoring_ a config, but never in _running_ one; the
