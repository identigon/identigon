...
automatically with a `COUNT(DISTINCT)` cardinality gate. That gate was a k-anonymity-era residue
and, critically, the **last place the privacy path read source values** - a dataset-level probe
intended to be removed entirely.
...
- A policy-author-declared boolean `distinguishing` flag, validated at config time, with an advisory
  (non-gating) cardinality lint as a safety net.
