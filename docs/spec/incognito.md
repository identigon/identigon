...
a plausible-but-obviously-fake value generated by [`alterego`](../../alterego) from a secret
per-run salt that is destroyed on completion. Because no real identifier or quasi-identifier value
survives, there is nothing to link a row back to a real individual; and because the replacements are
drawn from deliberately fictional vocabularies (fictional surnames, RFC 2606 reserved e-mail
...
a k-anonymity-era assumption: forming equivalence classes needs a _global_ pass over a table. With
...
> shifting - is delegated to `alterego`; Incognito only _decides_ (from policy and schema)
...
> primitive
> to AlterEgo, not to hand-roll it here.
>
...
has no built-in _fictional_ mapping below and no typed hint fails fail-closed with
