  ...
  void typedHintSynthesisesAFictionalCity() throws Exception {
    ...
    DataSource[] ds =
        freshDatabases(
            "city",
            ddl,
            "INSERT INTO people (city) VALUES ('Q')"); // 1-char source: shape-preserving would stay
    // 1 char
            ...
            .table(
                ...
                t ->
                    t.column("id", ColumnRole.PRIMARY_KEY, SurrogateStrategy.SEQUENTIAL_LONG)
                        .column(
                            ColumnPolicy.builder("city")
                                .role(ColumnRole.QUASI_ID)
                                .quasiIdStrategy(QuasiIdStrategy.SYNTHESISE)
                                .directIdStrategy(DirectIdStrategy.ALTEREGO_CITY)
                                .build()))
            .build();
  ...
  void unmappedTypeSynthesiseFailsClosed() throws Exception {
    ...
    IncognitoException.ConfigException ex =
        assertThrows(
            ...
            () -> run(ds, policy),
            "SYNTHESISE on an unmapped (numeric) type must fail closed");
  ...
  void characterTypeSynthesiseWithNoHintFailsClosed() throws Exception {
    ...
    String ddl =
        """
            ...
            """;
    DataSource[] ds =
        freshDatabases("postcode", ddl, "INSERT INTO addresses (postcode) VALUES ('SW1A 1AA')");
    ...
    IncognitoException.ConfigException ex =
        assertThrows(
            IncognitoException.ConfigException.class,
            () -> run(ds, policy),
            "SYNTHESISE on a hint-less character type must fail closed");
    assertTrue(ex.getMessage().contains("SYNTHESISE"), "message names the offending strategy");
