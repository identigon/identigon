...
class PostgresDialectHandlerTest {
    ...
    private static PreparedStatement recordingPreparedStatement(List<String> calls) {
        InvocationHandler handler = (proxy, method, args) -> switch (method.getName()) {
            case "setObject" -> {
                calls.add(args.length == 3
                    ? "setObject(" + args[0] + ", " + args[1] + ", " + sqlTypeName((int) args[2]) + ")"
        ...
        return (PreparedStatement) Proxy.newProxyInstance(
            PostgresDialectHandlerTest.class.getClassLoader(), new Class<?>[] {PreparedStatement.class}, handler);
    }
