...
/**
 * Regression test: {@link PostgresDialectHandler#preLoadTable} (owner-mode fallback), {@link
 * PostgresDialectHandler#postLoadTable}, and {@link PostgresDialectHandler#resyncSequence} must
 * quote identifiers, not just {@link PostgresDialectHandler#buildInsertSql} and the FK-drop/
...
class PostgresDialectHandlerIdentifierQuotingE2ETest {
  ...
  void setUp() throws Exception {
    ...
    pg =
        new PostgreSQLContainer(TestPostgres.IMAGE)
            .withDatabaseName("identifier_quoting")
            .withUsername("test")
            .withPassword("test");
    pg.start();
  ...
  void resyncSequenceQuotesTheTableAndColumn() throws SQLException {
    ...
    try (Statement stmt = ownerConn.createStatement();
      ...
      rs.next();
      assertEquals(
          3, rs.getLong(1), "sequence must resync past the two existing rows, not stay at 1");
    }
  }
}
