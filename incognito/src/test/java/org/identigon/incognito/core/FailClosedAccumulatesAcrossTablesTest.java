...
/**
 * A schema with fail-closed issues in more than one table reports all of them in a single run, not
 * one table (or one issue) per run - {@link SchemaDiscoveryStage#validateTablePolicy} accumulates
 * across every table and every check instead of throwing on the first hit. H2 in-memory, no Docker
 * needed - this is config validation, before any row is read.
 */
class FailClosedAccumulatesAcrossTablesTest {
  ...
  void unclassifiedColumnsInTwoTablesAreBothReportedInOneRun() throws SQLException {
    ...
    assertTrue(
        ex.getMessage().contains("CUSTOMERS") && ex.getMessage().contains("STATUS"),
        "names the CUSTOMERS.STATUS issue: " + ex.getMessage());
    assertTrue(
        ex.getMessage().contains("ORDERS") && ex.getMessage().contains("TOTAL"),
  ...
  void differentCheckKindsInDifferentTablesAreBothReportedInOneRun() throws SQLException {
    ...
    assertTrue(
        ex.getMessage().contains("DIRECT_ID") && ex.getMessage().contains("EMAIL"),
        "names the DIRECT_ID/EMAIL issue: " + ex.getMessage());
    assertTrue(
        ex.getMessage().contains("SENSITIVE") && ex.getMessage().contains("NOTES"),
