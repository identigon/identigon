...
/** Locks the {@link JsonWriter} contract (comma placement, nesting, escaping) independently of the emitter. */
class JsonWriterTest {
    ...
    void nullStringFieldWritesNullLiteral() {
        assertEquals("{\"x\": null}", new JsonWriter().beginObject().field("x", (String) null).endObject().toJson());
    }
    ...
    void escapesControlCharactersAsUnicode() {
        String json = new JsonWriter().beginObject().field("c", String.valueOf((char) 1)).endObject().toJson();
        assertEquals("{\"c\": \"\\u0001\"}", json);
    }
}
