 ...
 * internal clone (via {@code close()}, after which the instance is unusable). Runs the destruction
 * in {@code DefaultIncognitoPipeline}'s {@code finally}, so it holds whether the run succeeds or
 * fails. No Docker required.
 */
class SaltLifecycleTest {
  ...
  void saltIsZeroedAndAlterEgoClosedOnCompletion() {
    ...
    PipelineContext ctx =
        new DefaultPipelineContext(null, null, null, null, alterEgo, null, new HashMap<>());
