 ...
 * only the ones this run itself inserted (v3.1.0-era tutorial-feedback finding: the only warning on
 * this used to live in a documentation aside, not the tool). {@code --force} (@code
 * Builder.allowNonEmptyTarget()}) opts out. H2 in-memory, no Docker/Testcontainers needed - this
 * check never reads a fabricated value, only row counts.
 */
