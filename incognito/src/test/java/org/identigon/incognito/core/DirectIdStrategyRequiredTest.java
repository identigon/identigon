 ...
 * schema-discovery time (ADR 29), the same way an undeclared SENSITIVE {@code distinguishing}
 * flag already does. H2 in-memory, no Docker/Testcontainers needed - this is config validation in
 * {@link SchemaDiscoveryStage}, before any row is read.
 */
class DirectIdStrategyRequiredTest {
    ...
    void directIdWithNoStrategyFailsClosed() throws SQLException {
        ...
        IncognitoException.ConfigException ex = assertThrows(IncognitoException.ConfigException.class,
            () -> run(src, tgt, policy));
        assertTrue(ex.getMessage().contains("DIRECT_ID") && ex.getMessage().contains("directIdStrategy"),
    ...
    void directIdWithExplicitAlteregoGenericPasses() throws SQLException {
        ...
        AnonymisationPolicy policy = AnonymisationPolicy.builder()
            .table("CUSTOMERS", t -> t
                .column("ID", ColumnRole.PRIMARY_KEY, SurrogateStrategy.SEQUENTIAL_LONG)
                .column("BANK_ACCOUNT", ColumnRole.DIRECT_ID, DirectIdStrategy.ALTEREGO_GENERIC))
            .build();
