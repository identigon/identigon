 ...
 *
 * <ul>
 *   <li>a <b>non-superuser</b> target with cyclic FKs (cannot suppress FK enforcement) -> fail
 *       fast;
 *   <li>a <b>composite FK referencing a cyclic table</b> -> not yet supported;
 *   <li>a <b>cyclic FK on a table with no single-column PK</b> -> nothing to key the pass-2 UPDATE
 *       on.
 * </ul>
 *
...
class FailClosedGuardE2ETest {
  ...
  private void admin(String sql) throws SQLException {
    exec(jdbcBase + "postgres", "test", "test", sql);
  }
  ...
  /**
   * Creates a fresh source+target DB pair (owned by superuser {@code test}); loads {@code
   * sourceSql} into the source and {@code targetDdl} into the target.
   */
  ...
  void nonSuperuserWithCyclicFkFailsFast() throws Exception {
    ...
    IncognitoException.ConfigException ex =
        assertThrows(
            ...
            () ->
                run(
                    ds("nosuper_source", "test", "test"),
                    ds("nosuper_target", "limited", "lim"),
                    policy));
    assertTrue(
        ex.getMessage().contains("cyclic"),
        "message names the cyclic-FK cause: " + ex.getMessage());
  }
  ...
  void compositeFkToCyclicTableFailsClosed() throws Exception {
    ...
    newPair(
        "comp",
        ddl + "INSERT INTO tree (grp, id, pgrp, pid) VALUES (1, 1, 1, 2), (1, 2, 1, 1);",
        ddl);
    ...
    AnonymisationPolicy policy =
        AnonymisationPolicy.builder()
            .table(
                "tree",
                t ->
                    t.column("grp", PAYLOAD)
                        .column("id", PAYLOAD)
                        .column(
                            ColumnPolicy.builder("pgrp")
                                .role(FOREIGN_KEY)
                                .references("tree", "grp")
                                .build())
                        .column(
                            ColumnPolicy.builder("pid")
                                .role(FOREIGN_KEY)
                                .references("tree", "id")
                                .build()))
            .build();
    ...
    assertTrue(
        ex.getMessage().contains("composite") && ex.getMessage().contains("cyclic"),
  ...
  void cyclicFkWithoutPrimaryKeyFailsClosed() throws Exception {
    ...
    AnonymisationPolicy policy =
        AnonymisationPolicy.builder()
            .table(
                "loops",
                t ->
                    t.column("uid", PAYLOAD)
                        .column(
                            ColumnPolicy.builder("boss")
                                .role(FOREIGN_KEY)
                                .references("loops", "uid")
                                .build()))
            .build();
    ...
    assertTrue(
        ex.getMessage().contains("primary key") || ex.getMessage().contains("cyclic"),
