 ...
 *
 * <p>{@code region}'s real PK is {@code (country, code)} (2 columns), with a separate, wider {@code
 * UNIQUE (country, code, variant)} (3 columns); {@code district}'s composite FK references that
 * 3-column constraint, not the PK. Before this fix, {@code buildFkTransformer} iterated only over
 * the (fewer) PK columns to build the lookup key, silently dropping the FK's extra {@code variant}
 * column instead of detecting the mismatch - the old {@code orderedChildCols.contains(null)} check
 * only ever saw entries for the columns it *did* iterate, so a composite FK that fully covered the
 * PK (as this one does, plus one column more) slipped through with no null anywhere.
 *
    ...
    try {
      pg =
          new PostgreSQLContainer(TestPostgres.IMAGE)
              .withDatabaseName("wider_fk_source")
              .withUsername("test")
              .withPassword("test");
      pg.start();
      ...
      try (Connection conn =
        ...
        try (Statement stmt = conn.createStatement()) {
          ...
          stmt.execute(
              "INSERT INTO district (country, code, variant, district_no) VALUES ('GB', 1, 0, 1)");
        }
      }
                ...
                t.column(
                    ...
                    .column(
                        ColumnPolicy.builder("code")
                            ...
                            .build())
                    .column(
                        ColumnPolicy.builder("variant")
                            .role(ColumnRole.FOREIGN_KEY)
                            .references("region", "variant")
                            .build())
                    .column(
                        "district_no",
                        ColumnRole.PRIMARY_KEY,
                        SurrogateStrategy.PASSTHROUGH_SURROGATE))
        .build();
  }
        ...
        assertThrows(
            ...
            () ->
                IncognitoPipeline.builder()
                    ...
                    .build()
                    .execute(),
            "a composite FK wider than the parent PK must fail closed, not silently drop the extra "
                + "column");
