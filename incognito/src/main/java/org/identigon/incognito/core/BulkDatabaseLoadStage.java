...
/**
 * Handles batch insertion of transformed rows into the target database. Isolates dialect-specific
 * trigger and foreign key enforcement mechanics.
 */
public final class BulkDatabaseLoadStage implements AutoCloseable {
  ...
  /**
   * A pending pass-2 {@code UPDATE} that resolves one cyclic/self-referential FK once the
   * referenced row's surrogate key is known.
   *
   ...
   * key for batching {@link #resolveDeferredCyclicFKs}: every update sharing one of these can share
   * one {@link PreparedStatement} and one {@code executeBatch()}.
   *
   * @param tableName the table to update
   * @param pkColumn the PK column identifying the row to update
   * @param fkColumn the FK column to set
   ...
   * surrogates. Groups updates by {@link UpdateShape} - {@code tableName}/{@code pkColumn}/ {@code
   * fkColumn} fully determine an update's SQL text - so each distinct shape uses one {@link
   * PreparedStatement} and batches of up to {@link #BATCH_SIZE}, rather than a fresh
   ...
   */
  public static void resolveDeferredCyclicFKs(
      org.identigon.incognito.api.PipelineContext context, List<DeferredUpdate> deferredUpdates)
      throws org.identigon.incognito.api.IncognitoException {
    if (deferredUpdates == null || deferredUpdates.isEmpty()) {
      return;
    }
    ...
    try (Connection targetConn = context.target().getConnection()) {
      ...
      throw new org.identigon.incognito.api.IncognitoException.SchemaException(
          "Failed to resolve deferred cyclic FKs", e);
    }
  }
}
