...
/**
 * Serialises the {@link AnonymisationReport} to a DPIA artefact. Three formats are offered (SPEC §7
 * / PLAN Phase 6): machine-readable {@link #emitJson JSON}, presentation-ready {@link #emitHtml
 * HTML}, and human-diffable {@link #emitMarkdown Markdown}. All are zero-dependency (no JSON/HTML
 * library) so the core stays dependency-lean.
 *
 ...
 * wants a persisted DPIA artefact invokes one of these methods with that report - e.g. {@code
 * DpiaArtefactEmitter.emitJson(result.report(), path)}.
 */
public final class DpiaArtefactEmitter {
  ...
  private static final String HTML_HEAD =
      """
        ...
        .fail{color:#b00}
        .ok{color:#080}
        caption{font-weight:bold;text-align:left;padding:.3rem 0}
        </style>
        </head><body>
   ...
   */
  public static void emitJson(AnonymisationReport report, Path outputPath)
    ...
    for (AnonymisationReport.StructuralUniquenessFinding suf : report.structuralFindings()) {
      ...
      for (String col : suf.childColumns()) {
        jw.value(col);
      }
      jw.endArray();
   ...
   * @throws IncognitoException if writing fails
   */
  public static void emitHtml(AnonymisationReport report, Path outputPath)
      throws IncognitoException {
    try (Writer w = Files.newBufferedWriter(outputPath)) {
      w.write(HTML_HEAD);

      w.write(
          "<p><b>Salt mode:</b> <code>"
      ...
      if (report.survivalFindings().isEmpty()
          && report.lintFindings().isEmpty()
          && report.structuralFindings().isEmpty()) {
        w.write(
            "<p class=\"ok\">No source-value survival, misdeclaration, or structural"
                + " findings.</p>\n");
      }
      if (!report.survivalFindings().isEmpty()) {
        w.write(
            "<table><caption>Source-value survival (SPEC &sect;4.3 &mdash; singling-out evidence)"
        ...
        for (AnonymisationReport.SurvivalFinding sf : report.survivalFindings()) {
          w.write(
              "<tr><td>"
                  + htmlEscape(sf.table())
                  + "</td><td>"
                  + htmlEscape(sf.column())
                  + "</td><td>"
                  + sf.sampledDistinct()
                  + "</td><td>"
                  + sf.survived()
                  + "</td><td class=\""
                  + (sf.hardFailure() ? "fail\">LEAK" : "ok\">coincidental")
                  + "</td></tr>\n");
        }
        w.write("</table>\n");
      }
      if (!report.lintFindings().isEmpty()) {
        w.write(
            "<table><caption>Misdeclaration lint (SPEC &sect;4.1 &mdash; distinguishing:false"
      ...
      if (!report.structuralFindings().isEmpty()) {
        w.write(
            "<table><caption>Structural re-identification risk (SPEC &sect;2.4 &mdash;"
                ...
                + "<th>Unique fingerprints</th><th>Rare fingerprints (&lt;k)</th><th>k</th>"
                + "</tr>\n");
        for (AnonymisationReport.StructuralUniquenessFinding suf : report.structuralFindings()) {
          w.write(
              "<tr><td>"
                  + htmlEscape(suf.parentTable())
                  + "</td><td>"
                  + htmlEscape(suf.childTable())
                  + "</td><td>"
                  + htmlEscape(String.join(", ", suf.childColumns()))
                  + "</td><td>"
                  + suf.distinctParents()
                  + "</td><td>"
                  + suf.maxChildCount()
                  + "</td><td>"
                  + suf.uniqueFingerprintCount()
                  + "</td><td>"
                  + suf.rareFingerprintCount()
                  + "</td><td>"
                  + suf.k()
                  + "</td></tr>\n");
        }
      ...
      for (PipelineStage.StageResult sr : report.stageResults()) {
        w.write(
            "<tr><td>"
                + htmlEscape(sr.stageName())
                + "</td><td class=\""
                + (sr.success() ? "ok\">OK" : "fail\">FAILED")
                + "</td><td>"
                + sr.processedCount()
                + "</td><td>"
                + htmlEscape(sr.message())
                + "</td></tr>\n");
      }
      w.write("</table>\n<h2>Tables</h2>\n");
      if (report.tables().isEmpty()) {
        w.write("<p>No tables processed.</p>\n");
      }
      ...
      for (AnonymisationReport.TableReport tr : report.tables()) {
        w.write(
            "<h3><code>"
                ...
                + "<p>Rows processed: "
                + tr.rowsProcessed()
                + " &middot; Fictionality verified"
                + " (table-level - no verification failure found): "
        ...
        w.write(
            "<table><caption>Column actions</caption><tr><th>Column</th><th>Role</th>"
                + "<th>Transformation</th><th>Fictionality verified</th></tr>\n");
        for (AnonymisationReport.ColumnAction ca : tr.columns()) {
          w.write(
              "<tr><td>"
                  + htmlEscape(ca.column())
                  + "</td><td>"
                  + ca.role()
                  + "</td><td>"
                  + htmlEscape(ca.transformation())
                  + "</td><td class=\""
                  + (ca.fictionalityVerified() ? "ok\">yes" : "\">&mdash;")
                  + "</td></tr>\n");
        }
        w.write("</table>\n");
        if (!tr.columns().isEmpty() && !tr.columns().get(0).examples().isEmpty()) {
          w.write(
              "<table><caption>Sample rows (illustrative &mdash; synthetic data, not this"
        ...
        if (!tr.passthroughFlags().isEmpty()) {
          w.write(
              "<table><caption>Passthrough flags (opaque types kept as-is)</caption>"
                  + "<tr><th>Column</th><th>JDBC type</th><th>Reason</th></tr>\n");
          for (AnonymisationReport.PassthroughFlag pf : tr.passthroughFlags()) {
            w.write(
                "<tr><td>"
                    + htmlEscape(pf.column())
                    + "</td><td>"
                    + htmlEscape(pf.jdbcType())
                    + "</td><td>"
                    + htmlEscape(pf.reason())
                    + "</td></tr>\n");
          }
          w.write("</table>\n");
        }
        if (!tr.inferSuggestions().isEmpty()) {
          w.write(
              "<table><caption>Inference suggestions (surfaced only, never auto-applied)"
                  + "</caption><tr><th>Column</th><th>Suggested role</th><th>Heuristic</th>"
                  + "</tr>\n");
          for (AnonymisationReport.InferSuggestion is : tr.inferSuggestions()) {
            w.write(
                "<tr><td>"
                    + htmlEscape(is.column())
                    + "</td><td>"
                    + is.suggestedRole()
                    + "</td><td>"
                    + htmlEscape(is.matchedHeuristic())
                    + "</td></tr>\n");
          }
          w.write("</table>\n");
        }
  ...
  private static String saltModeNote(org.identigon.incognito.api.SaltMode mode) {
    if (mode == null) {
      return "salt mode not recorded";
    }
    return switch (mode) {
      case EPHEMERAL ->
          "fresh per-run salt, destroyed on completion; output unlinkable and irreversible";
      case PERSISTENT ->
          "fixed reused salt; output is linkable across runs and forfeits irreversibility (SPEC "
              + "§5.2)";
      case REPRODUCIBLE ->
          "fixed salt + seed for reproducible fixtures; linkable and not for production clones "
              + "(SPEC §5.2)";
    };
  }
   ...
   * @throws IncognitoException if writing fails
   */
  public static void emitMarkdown(AnonymisationReport report, Path outputPath)
      throws IncognitoException {
      ...
      if (!report.structuralFindings().isEmpty()) {
        writer.write(
            """
                    ...
                    | Parent Table | Child Table | FK Column(s) | Distinct Parents \
