...
public class TableDependencyGraph {
  ...
  public record TopologicalExecutionPlan(
      List<String> sequentialTableOrder, List<String> cyclicTablesToUpdatePass2) {}
   ...
   */
  public TopologicalExecutionPlan computeTopologicalOrder(
      List<SchemaInspector.TableMetadata> metadata) throws IncognitoException.SchemaException {
    ...
    while (!queue.isEmpty()) {
      ...
      for (String t : comp) {
        // Only include tables that are actually in the metadata (we might have edges to tables not
        // selected)
