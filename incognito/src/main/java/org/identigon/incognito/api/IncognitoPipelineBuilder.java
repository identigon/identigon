  ...
  public IncognitoPipeline build() {
    ...
    byte[] effectiveSalt =
        switch (resolvedSaltMode) {
          ...
          case REPRODUCIBLE ->
              this.salt == null ? null : deriveReproducibleSalt(this.salt, this.seed);
        };
    ...
    org.identigon.alterego.AlterEgo alterEgo =
        org.identigon.alterego.AlterEgo.builder()
            .salt(effectiveSalt)
            .locale(this.locale)
    ...
    KeyTranslationStore resolvedKeyStore =
        this.keyStore != null
            ? this.keyStore
            : new org.identigon.incognito.core.InMemoryKeyTranslationStore();

    PipelineContext context =
        new org.identigon.incognito.core.DefaultPipelineContext(
            source,
            target,
            resolvedKeyStore,
    ...
    context
        .attributes()
        .put(
            org.identigon.incognito.core.AnonymisationReportBuilder.ATTR_SALT_MODE,
            resolvedSaltMode);
    ...
    List<PipelineStage> resolvedStages =
        stages.isEmpty()
            ? List.of(
                new org.identigon.incognito.core.SchemaDiscoveryStage(),
    ...
    return new org.identigon.incognito.core.DefaultIncognitoPipeline(
        context, resolvedStages, effectiveSalt);
  }

  /**
   * Derives the salt actually used for a {@code reproducible(salt, seed)} run: {@code SHA-256(salt
   * || seed)}. Deterministic in {@code (salt, seed)} - same inputs always yield the same output -
   * so a fixed salt and seed remain byte-for-byte reproducible across runs, while varying the seed
   * alone (holding salt fixed) varies the fabricated output, matching SPEC §5.2. Package-private
   * (not private) so a unit test can exercise it directly, without a live database.
   */
  ...
  private void setSaltMode(SaltMode mode) {
    if (saltMode != null && saltMode != mode) {
      throw new IncognitoException.ConfigException(
          "salt modes are mutually exclusive: "
              + saltMode
              + " already set, cannot also set "
              + mode);
    }
    saltMode = mode;
  }
