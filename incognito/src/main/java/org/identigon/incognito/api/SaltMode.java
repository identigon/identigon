...
/**
 * How a run derived the salt that keys all fabrication (SPEC §5.1/§5.2). Recorded on the {@link
 * AnonymisationReport} because it is the single most safety-critical fact a DPIA reviewer needs to
 * weigh the Recital 26 "means reasonably likely to be used" test: two runs with identical
...
public enum SaltMode {
   ...
   * fixtures (SPEC §5.2). Carries the same linkability/irreversibility caveats as {@link
   * #PERSISTENT}, and is intended for test data, not production clones.
   */
  REPRODUCIBLE
}
