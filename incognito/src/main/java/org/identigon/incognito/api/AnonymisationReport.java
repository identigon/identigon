 ...
 * @param survivalFindings DIRECT_ID columns where real source values reappeared in the target (SPEC
 *     §4.3) - the quantified singling-out evidence (Art. 29 WP 05/2014) a reviewer needs
 * @param lintFindings {@code SENSITIVE distinguishing: false} columns whose real cardinality
 *     exceeds the categorical threshold (SPEC §4.1) - candidate misdeclarations kept opaque;
 *     populated only when the lint runs in {@link DistinguishingLint#WARN} mode (in {@code ERROR}
 *     mode the run aborts before any report is built)
 * @param structuralFindings per-FK-edge relational fingerprints (SPEC §2.4) - subjects singled out
 *     by their FK fan-out rather than by any field value; populated only when {@link
 *     StructuralUniquenessMode#REPORT} is enabled (off by default)
...
public record AnonymisationReport(
   ...
   *     (e.g. {@code ALTEREGO_GENERIC}), since nothing failed for it either. Each {@link
   *     ColumnAction}'s own {@link ColumnAction#fictionalityVerified()} is the precise, per-column
   *     answer (SPEC §4.3)
   */
   ...
   * @param transformation the transformation applied, e.g. {@code "ALTEREGO_EMAIL"} or {@code
   *     "KEEP"}
   ...
   *     {@code false} for every other column, including a deliberately-chosen {@code
   *     ALTEREGO_GENERIC} strategy - that fabrication is legitimate (ADR 21/29) but carries no
   *     fictionality guarantee to verify, so it is never reported as one
   */
   ...
   *
   * @param column the column name
   * @param jdbcType the JDBC type name that could not be transformed (e.g. {@code "OTHER"}, {@code
   *     "ARRAY"})
   ...
   * categorical-cardinality threshold (SPEC §4.1) - a candidate misdeclaration: the column was kept
   * opaque as low-cardinality categorical data but looks high-cardinality enough to be
   * quasi-identifying. This is a safety net, never the privacy gate (ADR 16) - the {@code
   * distinguishing} declaration alone decides keep-vs-fabricate.
   *
   * @param table the table name
   * @param column the column name
   ...
   * preserved 1:1, so a parent row with a rare or unique count of referencing child rows (e.g. "the
   * one customer with 300 orders") can be singled out from <em>structure</em> alone, even though
   * every field on it was fabricated. This is advisory evidence, never a privacy gate - it changes
   * no data and never fails the run (there is no {@code ERROR} mode).
   *
   ...
   * @param childColumns the child's FK column(s), in key order - identifies <em>which</em> edge
   *     this is when a child references the same parent through more than one foreign key
   ...
   * @param rareFingerprintCount how many parent rows have a child count shared by fewer than {@code
   *     k} parents in total (so a unique fingerprint, group size 1, is also rare when k &gt; 1)
  ...
  public record StructuralUniquenessFinding(
      ...
      long distinctParents,
      long maxChildCount,
      long uniqueFingerprintCount,
      long rareFingerprintCount,
      int k) {}
}
