 ...
 */
public record TablePolicy(String tableName, Map<String, ColumnPolicy> columns) {
  /**
  ...
  public static class Builder {
     ...
     *
     * @param columnName the column name (for readability; the built policy's own name is used as
     *     the key)
    ...
    public Builder column(String columnName, ColumnRole role, SurrogateStrategy strategy) {
      return column(
          ColumnPolicy.builder(columnName).role(role).surrogateStrategy(strategy).build());
    }

    /**
     * Declares a {@code DIRECT_ID} / {@code UNIQUE_CANDIDATE_KEY} column with its alterego
     * generator.
     *
     * @param columnName the column name
     * @param role the column role
    ...
    /**
     * Declares a {@code distinguishing: true} {@code SENSITIVE} column redacted via clear / mask /
     * constant (SPEC §2.2/§4.1).
     *
     * @param columnName the column name
    ...
    public Builder column(String columnName, ColumnRole role, RedactionStrategy strategy) {
      return column(
          ColumnPolicy.builder(columnName)
              .role(role)
              .distinguishing(true)
