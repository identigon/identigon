...
a plausible-but-obviously-fake value generated by [`lib-alterego`](../lib-alterego) from a secret
...
must-not-regress invariants (§7.3) define what Incognito does; the appendices pin the `lib-alterego`
integration, the
...
4. **Exact counts.** Volumes are *similar*, not identical. Records near a period boundary may shift
...
Incognito and [`lib-alterego`](../lib-alterego) are deliberately separate libraries, and the split
...
> shifting — is delegated to `lib-alterego`; Incognito only *decides* (from policy and schema)
...
- **Value-equality pattern preserved.** Because fabrication is deterministic per `(salt, value)`,
  ...
  consistency where a value is denormalised across tables). The *pattern* of which rows share a
  value is therefore preserved even though the values are fake. A per-column *non-deterministic*
...
The heart of Incognito is a **per-column transformation dial**, all of it backed by `lib-alterego`.
...
- **High-cardinality `UNIQUE_CANDIDATE_KEY` sequence fallback.** When `AlterEgo.unique()`'s
  ...
  *overlaid on the fabricated value's tail* so the original length is preserved exactly (never
  ...
  the deferred lib-alterego delegation, §1.4); a column whose format still cannot be satisfied is
...
a child has two *distinct* ancestor rows in the source table (a true fork, not a diamond to one
...
- **No `hashCode()`-derived fabricated values or jitter deltas.** The source value is known, so a
  ...
  fabricated value and every jitter delta derives from `lib-alterego`'s salt-keyed HMAC stream
...
- **Packages** `org.identigon.incognito.{api, core, spi, policy, engine}`; group id `org.identigon`;
  published **artifactId `incognito`** (distinct from the `lib-incognito` repository/directory
  name).
...
- **Dependencies:** `lib-alterego` (`api` — the field-transformation engine, exposed via
...
- **Delegation boundary (§1.4):** Incognito owns relational coherence and orchestration and
  delegates *all* field-value transformation to `lib-alterego` — it never hand-rolls substitution,
...
Verified against `../lib-alterego`. An implementer should not guess this API.
...
`ae.postcode()` — GB format only; lib-alterego ships no other country's postcode table yet, the same
