...
substitutes, such that the same input always produces the same output for a given configuration. It
is designed for use in Java streams, is extensible with custom transformations, and generates
...
`.map(...)`. Transformations are immutable and thread-safe: build one per built-in, reuse it across
every stream and thread you like.
...
- **Frequency is preserved.** Equal inputs map to equal outputs, so the most common surname in your
