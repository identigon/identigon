...
here so re-running the curation script (or picking the data apart differently later) doesn't depend
on these pages staying up or unchanged. All OGL v3.0 (Open Government Licence) - see
...
These are the raw files as published - `tools/curate_dictionaries.py` extracts specific ranks/years
from them by hand-copied values (see the script's own source comments for exactly which rows). They
are not read programmatically by the script itself.

Known limitation, already investigated and not worth re-chasing: see
`docs/research/0001-alterego-dictionaries.md`, "First names", for why Scotland and Northern Ireland
are single-cohort while England & Wales gets two.
