...
`AlterEgoConfigException` (throw at call time, not per element) for configuration problems:
...
consumers state the format they want. Built-ins that replace characters in place (email local parts,
phone digits) do so as documented per-transformation behaviour, not via a public inference facility.
...
dictionary-backed built-ins). `creditCardNumber()` is locale-independent (like
...
- **Determinism**: property tests asserting `t.apply(x)` is stable across calls, across stream order
  permutations, and between sequential and parallel streams.
...
- **Fictionality**: property tests assert every generated email uses a reserved domain, every UK
  ...
  wordlist — over large generated samples. For the identifier built-ins (section 4.8), the same
  style of test asserts, per output: `nhsNumber()` matches `999 \d{3} \d{4}` **and**
...
- **Locale equivalence**: `en-GB` and `cy-GB` configurations produce identical outputs for every v1
  built-in (country-scoped resolution, section 4).
- **Record coherence**: within a scope, town/postcode/phone agree whichever field is asked first;
  ...
  outside a scope behaviour is byte-identical to pre-scope behaviour; conflicting `set`
...
- **Dictionary coverage**: each shipped dictionary is non-empty, well-formed, its tag fields valid,
  ...
  `dictionaries/LICENCES/` (build-time check; section 9). Deduplicated means no duplicate (value,
  tags) row, not no duplicate value: a tagged dictionary may legitimately repeat a value under
  different tags (e.g. London spans several UK postcode areas, so it appears once per area) — only
  an exact repeated row is rejected.
...
- `purpose` is `"alterego/1/random"` for randomness keys, `"alterego/1/mapkey"` for mapping-store
  keys, and `"alterego/1/record"` for keyed record-attribute resolution (where the
