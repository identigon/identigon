...
/**
 * Typed, shared state for the fields of one record scope, reached from a strategy via {@link
 * TransformationContext#record()}. First touch wins: whichever field is transformed first fixes an
 * attribute, and later fields see that value. Outside any {@code RecordScope}, every method behaves
 * as a no-op: {@link #get} returns empty, {@link #set} is discarded, and {@link #computeIfAbsent}
 * resolves without retaining, so strategy code is identical in and out of a scope.
 */
public interface RecordAttributes {
   ...
   * attribute name, independent of which field asks first; in an anonymous scope, it receives the
  ...
  /**
   * Sets {@code key} to {@code value} if absent. Setting an already-fixed key to an equal value is
   * a no-op; setting it to a different value throws {@link AlterEgoCoherenceException}.
   *
   * @param <A> the attribute's value type
   * @param key the attribute key
   ...
   * whether attempting to *establish* a shared attribute (not just read one) is worthwhile at all,
   * since outside a scope nothing set or resolved is ever retained.
   *
